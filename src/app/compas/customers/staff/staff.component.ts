...
export class StaffComponent implements OnInit, OnDestroy {
    ...
    updateT4Customer(customer, status) {
        this.apiService.updateCustomerStatus(customer, status).subscribe(data => {
            ...
            return this.toastr.success('Profile details saved successfully. Awaiting authorization', ' Success!');
        }, error => {
        ...
        this.tellerSvc.addTeller(teller).subscribe((response) => {
            ...
            if (this.response.status === true) {
                if (teller.id === 0) {
                    ...
                    this.log(this.rightId, 'Updated Staff >StaffID: ' + teller.tellerId + '>UserName: ' + teller.tellerSignOnName);
                }
                this.customer = {};
                this.editMode = false;
                this.gtCustomers();
        ...
        this.apiService.addCustomer(this.customer).subscribe((response:any) => {
            ...
            if (this.response.status === true) {
                if (this.customer.id === 0) {
                    ...
                    this.log(this.rightId, 'Updated Customer >CIF: ' + this.customer.customerId + ' Cust Name: ' + this.customer.customerName);
                }
                this.customer = {};
                this.editMode = false;
                this.gtCustomers();
    ...
    removeCustomer(customer) {
        ...
        this.apiService.afisRemove(customerDetails).subscribe((response: any) => {
            ...
            if (res.status === true) {
                ...
                this.log(this.rightId, +'Failed to detached customer prints for customerId: ' + customerDetails.customerId + '.Enrollment Timeout');
            }
        }, error => {
            this.log(this.rightId, +'Failed to detach customer prints for customerId: ' + customerDetails.customerId + '.Enrollment Timeout');
        });
    }
